...
const UpdateLessonModal = ({
  ...
  useEffect(() => {
    if (lessonData && isOpen) {
      reset({
        ...
        duration: convertDurationToMinutes(lessonData.duration),
      });
    }
  }, [lessonData, isOpen, reset]);
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      formData.append("Order", "1"); // or dynamic order if needed
      formData.append("Type", data.type);
      formData.append("Duration", convertMinutesToHHMMSS(data.duration));
    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          >
            <h2 className="text-xl font-semibold mb-4 bg-[#7337FF26] p-3 text-center text-[#7337FF]">Update lesson</h2>
